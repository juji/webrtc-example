...
export type CaptureMode = "audio" | "video" | "photo";
...
const MODE_CONSTRAINTS: Record<CaptureMode, MediaStreamConstraints> = {
  audio: { audio: true },
  video: { audio: true, video: { facingMode: "environment" } },
...
export function CapturePopup({
  ...
  // Open the camera/mic as soon as a mode is picked, close it the instant the
  // popup goes away (mode becomes null) — never leave a device open in the background.
  useEffect(() => {
    if (!mode) return;
    setResult(null);
    setRecording(false);
    ...
    navigator.mediaDevices
      ...
      .catch((err) => console.error("capture device access failed:", err));
  ...
  function use() {
    ...
    const extension = mode === "photo" ? "png" : mode === "audio" ? "webm" : "webm";
      ...
      <div className="flex flex-col items-center gap-3">
        {mode === "audio" ? (
          result ? (
            ...
            <div className="flex aspect-square w-full max-w-sm items-center justify-center rounded-lg bg-black/5 dark:bg-white/5">
              <span className={`text-sm ${recording ? "text-red-500" : "text-zinc-500"}`}>
