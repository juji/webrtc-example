...
export default function ChatPage() {
  ...
  return <Chat selfId={user.id} selfUsername={user.username} peerUsername={peerUsername} />;
}
...
function Chat({
  ...
  // Reference-only page (plans/convo Phase 9 retires it) — no real peer id
  // available from the route (:username param only), so persistence to
  // webrtc-convos here won't be accurate. Not a live/routed page.
  const { connected, messages, sendMessage, sendFile } = useWebRtcChat(selfId, selfUsername, peerUsername, peerUsername);
          ...
          >
            {m.files[0] ? (
              m.files[0].type.startsWith("image/") ? (
                ...
                <img src={m.files[0].url} alt={m.files[0].name} className="max-h-60 max-w-full rounded" />
              ) : (
                <a href={m.files[0].url} download={m.files[0].name} className="underline">
                  {m.files[0].name}
                </a>
              )
            ) : (
