...
- [x] Verified server-side via curl: accept succeeds and returns the right contact, request status flips to `accepted`, repeat accept rejected (409), non-recipient accept attempt rejected (404), request no longer appears in `GET /contacts/requests` after acceptance
