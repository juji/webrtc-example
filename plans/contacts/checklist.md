...
- [x] **`RequestsPopup` renamed "Notifications," rewritten as a flat list** — each row is self-describing (other party + a direction/status-aware description sentence + status/Accept action), not split into categories. A tabbed Received/Sent layout was tried first and reverted on direct feedback: tabbing re-created "contact requests" under a name that promised something more general. Opening the popup re-syncs any already-`accepted` outgoing row via the new `syncAcceptedContact()` (`client/lib/contacts.ts`, extracted from `service-worker-registration.tsx`'s push handler so both call sites share one verify-then-persist implementation) — this is what makes the fix durable: push is now an optimization, not the only delivery path
...
- [x] **Bug found and fixed: BB's accept-notification click didn't open the popup.** The accept push's `url` was still `/chat` (pre-redesign, before BB had a real notification to deep-link to) instead of deep-linking. One-line fix in `server/src/routes/contacts.ts`'s accept handler.
- [x] **`?open=requests` renamed `?open=notifications`** — naming cleanup once the popup's own framing became generic; three call sites (`client/app/chat/page.tsx`, both push URLs in `server/src/routes/contacts.ts`).
